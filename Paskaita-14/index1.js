...
// You are given an array of numbers. Your task is to use the reduce method to calculate the sum of all numbers present in the array.
...
// Given an array of strings, use the reduce method to convert it into an object where each string becomes a key and its length becomes the value.
...
// Given an array of strings, use the sort method to sort the strings based on the last character in each string.
...
const sortedFruit = fruit1.sort((a, b) => {
  // console.log(a[a.length - 1].localeCompare(b[b.length - 1]));
});
...
const sorted = numbers1.sort((a, b) => {
  return a - b;
});
...
// Given an array of objects, where each object has a date property in the format YYYY-MM-DD, sort them in ascending order based on the date.
